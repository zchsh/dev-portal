import React from 'react'
import { ComponentMeta } from '@storybook/react'
import createLiveComponent from '__swingset-components/live-component'
import CollectionCard from 'views/tutorial-view/components/collection-card'

export default {
  title: 'Components/CollectionCard',
  component: CollectionCard,
  parameters: {
    docs: {
      components: {
        CollectionCard,
        LiveComponent: createLiveComponent({ CollectionCard }),
      },
    },
  },
} as ComponentMeta<typeof CollectionCard>

export const Playground = (args) => <CollectionCard {...args} />
Playground.args = {
  id: 'a0120d29-8aec-4cd1-a881-f58a85952edc',
  slug: 'vault/getting-started',
  numTutorials: 13,
  name: 'Getting Started',
  description:
    'Vault secures, stores, and tightly controls access to tokens, passwords, certificates, API keys, and other secrets in modern computing. Get started here.',
  theme: 'vault',
}

export const ExampleConsul = (args) => <CollectionCard {...args} />
ExampleConsul.args = {
  id: 'b0e74d7f-b7aa-497d-8314-5bf334862f3e',
  slug: 'consul/vault-secure',
  numTutorials: 7,
  name: 'Secure Consul with Vault Integrations',
  description: 'Integrate with Vault to enforce security best practices.',
  theme: 'consul',
}

export const ExampleVault = (args) => <CollectionCard {...args} />
ExampleVault.args = {
  id: 'a0120d29-8aec-4cd1-a881-f58a85952edc',
  slug: 'vault/getting-started',
  numTutorials: 13,
  name: 'Getting Started',
  description:
    'Vault secures, stores, and tightly controls access to tokens, passwords, certificates, API keys, and other secrets in modern computing. Get started here.',
  theme: 'vault',
}
